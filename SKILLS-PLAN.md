# C.H.A.R.L.I. — Skills & Integrations Plan
...
> 4. When in doubt, don't install it. CHARLI can often accomplish the task with built-in tools + bash.
...
These are bundled/official skills that directly support CHARLI's core roles. Install these first.

| Skill | Source | Purpose | CHARLI Role |
|---|---|---|---|
...
| Skill | Source | Purpose | Security Notes | CHARLI Role |
|---|---|---|---|---|
...
| **railway-skill** | Community (in awesome-openclaw-skills) | Deploy and manage applications on Railway.app. | Review for deploy triggers — CHARLI should NOT auto-deploy to production per the constitution. **Use a project-scoped Railway token.** | Business / SaaS Dev |
...
1. **Write a custom CHARLI skill** at `~/charli/skills/stripe/SKILL.md` that wraps the Stripe CLI (`stripe` — official CLI from Stripe).
...
3. **Prohibit write operations** (create charges, refunds, transfers) unless Sir explicitly instructs CHARLI per-task.
4. Use a **restricted Stripe API key** with only the permissions CHARLI needs — never the secret key.
5. This keeps your Stripe integration under your full control, auditable, and free of third-party risk.

### Cloudflare: Beyond R2

For broader Cloudflare management (Workers, Pages, DNS, etc.), consider using the **Wrangler CLI** directly via CHARLI's built-in exec tool rather than installing a skill. CHARLI can run `wrangler` commands natively — no skill needed, no token overhead. Just ensure `wrangler` is installed and authenticated on the Mac Mini.
...
| Category | Why |
|---|---|
| **Crypto/DeFi/trading skills** (bankr, token-deployment, polymarket) | Financial risk. CHARLI must never make financial transactions per the constitution. |
| **Social media posting skills** (clawk, bird/X) | Reputation risk. CHARLI should never post on your behalf without explicit control. |
...
1. **Can CHARLI do it with built-in tools?** Bash + exec + write covers a lot.
2. **Can it be an n8n workflow instead?** Free compute on the VPS.
3. **Can Sir write a custom skill?** A custom SKILL.md in `~/charli/skills/` is more trustworthy than any community skill and costs nothing.

CHARLI can even write its own skills. If you need a specific integration, describe it to CHARLI and have him generate a workspace skill in `~/charli/skills/`. You control the code, you review it, and it's free.

---

*Minimal surface area, maximum capability. That's the C.H.A.R.L.I. way, Sir.*
